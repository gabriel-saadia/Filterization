...
If $\X$ is a small vf-category, the set of its open subspaces determines a topology on $\X_0$: this construction yields a 2-functor $\Open \colon \vfCat \to \TopSp$ mapping each small vf-category to the topological space determined by its open subspaces and each vf-functor to its underlying map of points.
...
\subsection{Points of the localic reflection}Throughout this subsection, fix a topos $\E$. There are two vf-posets that we can canonically associate to $\E$:
\begin{enumerate}
