...
\begin{proof}
    First, note that it suffices to prove the claim under the additional hypothesis of $\prod_{c\in \Ob\mathcal C} A(c)$ being inhabited. Indeed, applying \Cref{lem:choice-up-to-filter} to the family $(A(c))_{c\in \mathcal C}$ we obtain a reduced power $A^\lambda \colon \mathcal C \to \Set$ with respect to some proper filter $\lambda$ which satisfies this extra assumption, in which case we obtain a further reduced power $(A^\lambda)^{\lambda'} \cong A^{\lambda' \cdot \lambda}\colon \mathcal C \to \Set$ admitting a global section for some proper filter $\lambda'$, so that $\lambda'\cdot \lambda$ proves the original claim for $A$.
...
\begin{proof}
    ...
    Fix a small dense subcategory $\mathcal G$ of $\E$ which, without loss of generality, we can assume to admit finite limits.\footnote{Constructively, we can suppose that $\E$ is given as the category of sheaves on a lex site, so that $\mathcal G$ can be taken as its underlying category.} By density, to define a natural $p \Rightarrow \int_{t:\tau} q_{\phi t}$ it suffices to define one between the restrictions $p\vert_{\mathcal G} \Rightarrow \int_{t:\tau} q_{\phi t}\vert_{\mathcal G}$. 
