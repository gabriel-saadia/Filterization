...
\begin{proof}
    Let $L$ be the set $\sum_{I_0 \in \mathcal P_\omega I} \prod_{i\in I_0} A_i$ of all partial choice functions for the family $(A_i)_{i\in I}$ with finite domain, where $\mathcal P_\omega I$ is the set of finite subsets of $I$. For each $i \in I$, consider the set 
    \[ L_i \coloneqq \set{ l \in L | i \in \dom (l)}\]
    of partial functions defined at $i$. Note that any finite intersection $L_{i_0} \cap \dots \cap L_{i_n}$ is inhabited since so is each $A_i$; therefore, the filter $\lambda$ generated by the sets $(L_i)_{i\in I}$ is proper. By construction, for each $i\in I$, the family $(l(i))_{l\in L_i}$ represents an element $(l(i))_{l:\lambda}$ of $A_i^\lambda$; an element of $\prod_{i\in I} A_i^\lambda$ is thus given by $((l(i))_{l: \lambda})_{i\in I}$.
\end{proof}
\end{lem}

\begin{prop}
    Let $\mathcal C$ be a small filtered category and let $A \colon \mathcal C \to \Set$ be a functor such that $A(c)$ is inhabited for each $c$ in $\mathcal C$. Then, there exists a proper filter $\lambda$ such that the functor $A^\lambda \colon \mathcal C \to \Set$ defined by $A^\lambda(c) \coloneqq A(c)^\lambda$ admits a global section. 
\begin{proof}
    First, note that it suffices to prove the claim under the additional hypothesis of $\prod_{c\in \mathcal C} A(c)$ being inhabited. Indeed, this `weaker' version can be applied to some reduced power $A^\lambda \colon \mathcal C \to \Set$ with respect to a proper filter $\lambda$, given by applying \Cref{lem:choice-up-to-filter} to the family $(A(c))_{c\in \mathcal C}$, obtaining that $(A^\lambda)^{\lambda'} \cong A^{\lambda \times \lambda'}\colon \mathcal C \to \Set$ admits a global section for some proper filter $\lambda'$. 
