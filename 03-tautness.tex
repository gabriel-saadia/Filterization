...
\begin{enumerate}
    \item[i.]on one hand, as we saw in \Cref{ex:points-of-toposes}, we can consider its canonical vf-category of points $\pt(\E)$; 
...
\end{enumerate}
Note then that there is an evident vf-functor $\pt (\E) \to \pt(\Lref{\E})$, defined by mapping each point $p \colon \E \to \Set$ to its restriction $p|_{\Sub_{\E}(1)} \colon \Sub_{\E}(1) \to \Omega$ to subterminal objects, which is a frame homomorphism and hence a point of $\Lref{\E}$. As $\Tref{-}$ is left adjoint to the inclusion of vf-posets into vf-categories, in particular we have a vf-functor $\Tref{\pt(\E)} \to \pt(\Lref{\E})$. In this subsection, we show that this vf-functor is an \emph{equivalence} of vf-posets.
