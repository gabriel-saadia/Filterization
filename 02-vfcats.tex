...
\begin{rmk}[Reduced powers of reduced products]\label{rmk:monoidal-structure-on-F}
    Recall by \cite[\S 4]{blassTwoClosedCategories1977} that the tensor product defines a monoidal closed structure on $\FF$:
    given two filters $(S,\sigma)$ and $(X,\xi)$, the internal hom-object $[\sigma, \xi]$ is the filter on the set $\FF(\sigma, X)$ generated by the family $\set{ \FF(\sigma, X_0) | X_0 \in \xi}$. This means that, in particular, if $\xi = \delta_X$, then $[\sigma, \xi] = \delta_{\FF(\sigma, X)}$. Thus, for any filter $\lambda$, we have a bijection
    \[ \FF(\lambda, \FF(\sigma, X)) \cong \FF(\lambda \cdot \sigma , X) \]
    or, in other words:
    \[ (X^\sigma)^\lambda \cong X^{\lambda \cdot \sigma}.\]
