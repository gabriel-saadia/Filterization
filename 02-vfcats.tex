...
We recall here the \emph{dependent product} of filters, combining an indexed family of filters with respect to a filter on the index set. In the non-dependent case, this reduces to the tensor product of filters considered already since \cite{blassTwoClosedCategories1977}; in the case where the filter on the index set is actually a set, this reduces instead to a description of coproducts in $\FF$. \todo{find a first ref for this too}

\begin{cons}[Dependent products]
    For a filter $\sigma$ on a set $S$ and a family $(\tau_s)_{s\in S}$ of filters, each on a set $T_s$, we denote by $\sigma \cdot \tau_s$ the filter on the set $\sum_{s\in S}T_s$ given by those subsets of the form $\sum_{s\in S_0} T_{s,0}$ where $S_0 \in \sigma$ and $T_{s,0} \in \tau_s$ for each $s \in S_0$. This construction subsumes both \emph{tensor products} and of \emph{coproducts} of filters.
\begin{itemize}
...
Consider an $S$-family $(X_s)_{s\in S}$ of sets and a filter $\sigma$ on the set $S$. Its \emph{reduced product} is the set 
\[ \int_{s:\sigma} X_s \coloneqq \colim_{ S_0 \in \nu^{\op}} \prod_{s\in S_0} X_s,\]
whose elements are $\nu$-equivalence classes of tuples $(x_s \in X_s)_{s\in S_0}$ for some $S_0 \in \nu$. In the case where all sets $X_s$ coincide with a single set $X$, we also speak of the \emph{reduced power} of $X$ with respect to $\sigma$, which we denote by $X^\sigma$. These constructions, well-known in model theory \cite{?}, generalize the notions of \emph{ultraproduct} and \emph{ultrapower} to arbitrary filters. Categorically, we now see how they give rise to functors $\FF^{\op} \to \Set$; we start with the simpler case of reduced powers.
...
To generalize \Cref{cons:reduced-powers}, note that for an $S$-family $(X_s)_{s\in S}$ of sets and a filter $\sigma$ on $S$, we can see both $\sigma$ and $\sum_{s\in S} X_s$ as objects of $\FF/S$, respectively via the identity map and the projection map. 

\begin{cons}[Reduced products]
    The hom-set $\FF/S (\sigma, \sum_{s\in S} X_s)$ is in bijection with the reduced product $\int_{s:\sigma} X_s$.
...
We introduce here an operation on vf-arrows obtained combining diagonals and compositions. For an $S$-family $(u_s \colon p \vuto{\tau_s} q_{s,t})_{s \in S}$ of vf-arrows with the same domain and a filter $\sigma$ on $S$, their \emph{merge} is the vf-arrow $p \vuto{\tau_i \cdot \sigma} q_{s,t}$ obtained as the composite
\[ u_s \circ_\sigma \diag^\sigma_p \colon p \vuto{\sigma} (p \vuto{\tau_s} q_{s,t}),\]
which we denote by $\bigmrg_{s:\sigma} u_s$. As shown by the following lemma, merging provides an inverse operation for specialization along the canonical maps into a coproduct of filters.

\begin{lem}
    Let $\mathscr{X}$ be a vf-category and fix a family $\set{ \sigma_i }_{i\in I}$ of filters. For each point $p$ and each family $(q_{i,s})_{(i,s)\in \sum_i S_i}$ of points, the map 
