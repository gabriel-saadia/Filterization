...
\begin{rmk}\label{rmk:monoidal-structure-on-F}
    Recall from \cite[\S 4]{blassTwoClosedCategories1977} that the tensor product defines a monoidal closed structure on $\FF$:
    given two filters $(S,\sigma)$ and $(T,\tau)$, the internal hom-object $[\sigma, \tau]$ is the filter on the set $\FF(\sigma, T)$ generated by the family $\set{ \FF(\sigma, T_0) | T_0 \in \tau}$. This means that, in particular, if $\tau = \delta_T$, then $[\sigma, \tau] = \delta_{\FF(\sigma, T)}$. Thus, for any filter $\lambda$, we have a bijection
    \[ \FF(\lambda, \FF(\sigma, T)) \cong \FF(\lambda \cdot \sigma , T) \]
    or, in other words:
    \[ (T^\sigma)^\lambda \cong T^{\lambda \cdot \sigma}.\]
    ...
    \[ \textstyle \FF(\lambda, \FF/S(\sigma, \sum_{s\in S} X_s)) \cong \FF/S(\lambda \cdot \sigma ,  \sum_{s\in S} X_s) \]
    or, in other words:
